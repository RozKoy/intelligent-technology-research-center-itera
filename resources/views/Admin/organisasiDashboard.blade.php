...
<div class="flex flex-col w-full min-h-screen">
	...
	<div class="p-3 flex flex-wrap justify-center">
		...
		@if ($kepala)
			<div class="flex w-3/5 sm:w-60 rounded-lg shadow-lg shadow-gold m-2 overflow-hidden bg-gray-50 flex-col">
				<div class="bg-[url({{ asset('storage/' . $kepala -> image) }})] bg-no-repeat bg-center bg-cover w-full aspect-[3/4]"></div>
				<div class="flex flex-col items-center font-semibold text-center">
					<p>{{ $kepala -> name }}</p>
					<hr class="w-4/5 my-1">
					<p>{{ $kepala -> position }}</p>
				</div>
				<a href="{{ url() -> current() . '/' . $kepala -> id }}" class="text-white bg-red-700 hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-red-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mx-auto my-2.5 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Hapus</a>
			</div>
		@endif
		@if (!$data -> isEmpty())
			@foreach ($data as $item)
				<div class="flex w-3/5 sm:w-60 rounded-lg shadow m-2 overflow-hidden bg-gray-50 flex-col">
					<div class="bg-[url({{ asset('storage/' . $item -> image) }})] bg-no-repeat bg-center bg-cover w-full aspect-[3/4]"></div>
					<div class="flex flex-col items-center font-semibold text-center">
						<p>{{ $item -> name }}</p>
						<hr class="w-4/5 my-1">
						<p>{{ $item -> position }}</p>
		...
		@if (!$kepala && $data -> isEmpty())
			<p class="text-red font-bold">Belum Ada Data Organisasi</p>
		@endif
